...
databaseChambre = "/tmp/remote_database_dir_chambre/Database/MyVideos90.db"
databaseSalon = '/tmp/remote_database_dir_salon/Database/MyVideos90.db'
...
pathChambre = "/tmp/remote_database_dir_chambre"
pathSalon = "/tmp/remote_database_dir_salon"
...
for row in data:
	...
	if dataSalon is None:
		...
		cur.execute("INSERT INTO bookmark (idFile, timeInSeconds, totalTimeInSeconds, thumbNailImage, player, playerState, type) VALUES (?,?,?,?,?,?,?)" , 
(bookmarkItem.idFile, bookmarkItem.timeInSeconds, bookmarkItem.totalTimeInSeconds, bookmarkItem.thumbNailImage, bookmarkItem.player, bookmarkItem.playerState, 
bookmarkItem.type))		
	else:
...
if os.path.exists(pathChambre) == True:
	os.popen("cp -r -f /tmp/remote_database_dir_salon/Database/MyVideos90.db /tmp/remote_database_dir_chambre/Database/")
